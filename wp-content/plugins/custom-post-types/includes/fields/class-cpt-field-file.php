...
class CPT_Field_File extends CPT_Field {
	...
	public static function render( $input_name, $input_id, $field_config ) {
		...
		<div class="cpt-file-section"
			data-type="<?php echo htmlspecialchars( wp_json_encode( $types ), ENT_QUOTES, 'UTF-8' ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>">
			<input name="<?php echo $input_name; ?>" value="<?php echo $field_config['value']; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>" class="cpt-hidden-input">
			<div class="cpt-file-wrap">
				...
				<div class="cpt-file-actions"
					title="<?php echo $field_config['value'] && get_post( $field_config['value'] ) ? basename( get_attached_file( $field_config['value'] ) ) : __( 'Choose', 'custom-post-types' ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>">
					<div class="file-name"
						dir="rtl"><?php echo $field_config['value'] && get_post( $field_config['value'] ) ? basename( get_attached_file( $field_config['value'] ) ) : ''; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div>
					<div class="buttons">
						<button class="button cpt-file-button button-primary cpt-file-upload" id="<?php echo $input_id; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>"
								title="<?php esc_attr_e( 'Choose', 'custom-post-types' ); ?>"
								aria-label="<?php esc_attr_e( 'Choose', 'custom-post-types' ); ?>">
