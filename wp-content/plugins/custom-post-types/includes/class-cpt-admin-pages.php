...
final class CPT_Admin_Pages extends CPT_Component {
	...
	private function render_callback( $id, $title, $content = false ) {
		...
		<div class="wrap cpt-admin-page">
			<h1 class="cpt-admin-page-title"><?php echo esc_html( $title ); ?></h1>
			<?php
			if ( ! empty( $content ) ) {
				printf(
					'<div class="cpt-admin-page-content">%s</div>',
					'tools' === $id ? $content : apply_filters( 'the_content', $content ) //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				);
			}
			ob_start();
